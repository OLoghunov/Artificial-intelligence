...
bool SurfaceGraph::initialize(const QSize &minimumGraphSize, const QSize &maximumGraphSize)
{
    ...
    auto *selectionGroupBox = new QGroupBox(u"SPO settings"_s);
    ...
    auto *particleCount = new QSlider(Qt::Horizontal);
    particleCount->setMinimum(50);
    particleCount->setTickInterval(1);
    particleCount->setEnabled(true);

    auto *maxIteration = new QSlider(Qt::Horizontal);
    maxIteration->setMinimum(1000);
    maxIteration->setMaximum(2000);
    maxIteration->setTickInterval(1);
    maxIteration->setEnabled(true);
    ...
    QLabel *partCountLabel = new QLabel("Particle count:\n50                     100");
    selectionVBox->addWidget(partCountLabel);
    selectionVBox->addWidget(particleCount);

    QLabel *maxIterationLabel = new QLabel("Maximum number\nof iterations:\n1000                  2000");
    selectionVBox->addWidget(maxIterationLabel);
    selectionVBox->addWidget(maxIteration);
    ...
    QObject::connect(particleCount, &QSlider::valueChanged,
                     modifier, &SurfaceGraphModifier::changeParticleCount);

    QObject::connect(maxIteration, &QSlider::valueChanged,
                     modifier, &SurfaceGraphModifier::changeMaxIteration);
    ...
    QObject::connect(itemStyleList, &QComboBox::currentIndexChanged,
                     modifier, &SurfaceGraphModifier::changeFunction);
