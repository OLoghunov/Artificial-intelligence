...
GASurfaceGraphModifier::GASurfaceGraphModifier(Q3DSurface *surface, QLabel *label, QObject *parent) :
    QObject(parent),
    m_graph(surface),
    m_textField(label)
{
...
void GASurfaceGraphModifier::fillSqrtSinProxy(int flag)
{
    ...
    for (int i = 0 ; i < sampleCountZ ; ++i) {
        ...
        for (int j = 0; j < sampleCountX; ++j) {
            ...
            {
                y = 100 * pow((z - pow(x, 2)), 2) + pow((1 - x), 2);
                y /= 20000.0;
            }
            else if (flag == 5)
            {
                y = 8 * pow(x, 2) + 4 * x * z + 5 * pow(z, 2);
                y /= 300.0;
            }
...
void GASurfaceGraphModifier::enableSqrtSinModel(bool enable)
{
    if (enable) {
        //! [3]
...
void GASurfaceGraphModifier::adjustXMin(int min)
{
...
void GASurfaceGraphModifier::adjustXMax(int max)
{
...
void GASurfaceGraphModifier::adjustZMin(int min)
{
...
void GASurfaceGraphModifier::adjustZMax(int max)
{
...
void GASurfaceGraphModifier::changeFunction(int f)
{
...
void GASurfaceGraphModifier::changeCoding(int f)
{
    m_codeNumber = f;
}

void GASurfaceGraphModifier::setAxisXRange(float min, float max)
{
    m_graph->axisX()->setRange(min, max);
}

void GASurfaceGraphModifier::setAxisZRange(float min, float max)
{
    m_graph->axisZ()->setRange(min, max);
}

void GASurfaceGraphModifier::setBlackToYellowGradient()
{
    //! [8]
    QLinearGradient gr;
...
void GASurfaceGraphModifier::setGreenToRedGradient()
{
    QLinearGradient gr;
...
void GASurfaceGraphModifier::handleElementSelected(QAbstract3DGraph::ElementType type)
{
    resetSelection();
...
void GASurfaceGraphModifier::resetSelection()
{
